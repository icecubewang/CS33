...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, arrayX, arrayY, seed) private(i)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}

#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, arrayX, arrayY, array, Ones, objxy, index, max_size, Y, Z, iter, probability) private(i, j, value, sub)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
   			index_X = round(arrayX[i]) + objxy[j*2 + 1];
   			index_Y = round(arrayY[i]) + objxy[j*2];
			value = fabs(index_X*Y*Z + index_Y*Z + iter);
   			index[i*Ones + j] = (max_size <= value) ? 0 : value;
   		}
   		probability[i] = 0;

   		for(j = 0; j < Ones; j++) {
		  sub = array[index[i*Ones + j]];
   			probability[i] += (pow((sub - 100),2) -
   							  pow((sub - 228),2))/50.0;
   		}
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(weights, probability, n) private(i, individualweight)
	for(i = 0; i < n; i++)
	  {
	    individualweight = weights[i] * exp(probability[i]);
	    weights[i] = individualweight;
	    sumWeights += individualweight;
	  }
	
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(weights, sumWeights, n) private(i)
	for(i = 0; i < n; i++)
