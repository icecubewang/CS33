...
int bitCount(int x) {
    ...
    int sum1 = (x&mask) + ((x>>1)&mask) + ((x>>2)&mask) + ((x>>3)&mask) + ((x>>4)&mask) + ((x>>5)&mask) + ((x>>6)&mask) + ((x>>7)&mask);
    return ((sum1&0xFF) + (sum1>>8&0xFF) + (sum1>>16&0xFF) + (sum1>>24&0xFF));
}
/* 
...
int isLessOrEqual(int x, int y) {
  return ((x>>31)&(!(y>>31))) |((((!(x>>31))&(!(y>>31))) | ((x>>31) & (y>>31))) & (!((y+(~x+1))>>31)));
}
/* 
